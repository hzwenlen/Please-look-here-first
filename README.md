# 找一找，记一记，写一写，存一存 2025.9.30.2136
</p>----------------------------------------------------------------------------------------------------------------</p>
...
Deployment Image Servicing and Management (DISM) 是一款命令行工具，用于修复您的 Windows 10 和 11 映像。它可以查找损坏或缺失的系统文件，并修复它们，以解决您的 PC 上的关键错误。
系统文件检查器工具可以帮助您修复Windows功能中的故障。它将扫描系统中的问题，并在检测到时恢复必要的文件以解决问题。您可以同时运行DISM和系统文件检查器工具以获得最佳结果。以下是操作方法。
要运行DISM和系统文件检查器工具：
按下Win键，然后输入cmd。
右键点击命令提示符，选择以管理员身份运行。
在命令提示符窗口中，输入以下命令来运行DISM工具：DISM.exe /Online /Cleanup-image /Restorehealth
运行系统文件检查器工具
键入以下命令并按Enter键运行系统文件检查器工具：sfc /scannow
验证可能需要几分钟时间。因此，请等待验证达到100%。返回结果将显示是否发现问题以及工具是否能够修复它。
输入exit并按下Enter键以关闭命令提示符。
</p>----------------------------------------------------------------------------------------------------------------</p>
Logo
</p>----------------------------------------------------------------------------------------------------------------</p>
