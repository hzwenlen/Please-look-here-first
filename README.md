...
:64BIT
set ARCH=amd64
set ASSIST_CLIENT_HOME=C:\ProgramData\AssistClient\
set DEST_BIN_FILE=%ASSIST_CLIENT_HOME%/assist-client
set VERSION_FILE=version
set METADATA_HOST_V4=http://100.96.0.96
set METADATA_HOST_V6=http://[fd00:64:0:200::6460:60]
set METADATA_HOST=%METADATA_HOST_V4%
set REGION_CODE=unknown
set METADATA_TOKEN=""

if exist "C:\Program Files\VolcStack\volc_cfg.json" (
  echo read metadata addr from file
  for /F %%i in ('powershell -command "$content=Get-Content 'C:\Program Files\VolcStack\volc_cfg.json' -Raw | ConvertFrom-Json; $content.metadataAddr"') do ( set METADATA_HOST_V4=%%i)
  for /F %%i in ('powershell -command "$content=Get-Content 'C:\Program Files\VolcStack\volc_cfg.json' -Raw | ConvertFrom-Json; $content.metadataAddrV6"') do ( set METADATA_HOST_V6=%%i)
)

powershell -Command "Invoke-RestMethod -Uri '%METADATA_HOST_V4%/latest/api/token' -Method Put  -Headers @{'X-volc-ecs-metadata-token-ttl-seconds' = '20'}"
if %errorlevel% neq 0 (
  GOTO TryMetadataV6
) else (
  set METADATA_HOST=%METADATA_HOST_V4%
...
:TryMetadataV6
echo try metadata addr %METADATA_HOST_V6%
powershell -Command "Start-Sleep -Milliseconds (Get-Random -Minimum 1000 -Maximum 5000)"
powershell -Command "Invoke-RestMethod -Uri '%METADATA_HOST_V6%/latest/api/token' -Method Put  -Headers @{'X-volc-ecs-metadata-token-ttl-seconds' = '20'}"
if %errorlevel% neq 0 (
   echo invalid metadata host
   exit 1
) else (
   set METADATA_HOST=%METADATA_HOST_V6%
)

:GetRegionId
echo METADATA_HOST %METADATA_HOST%
powershell -command "Start-Sleep -Milliseconds (Get-Random -Minimum 1000 -Maximum 5000)"
for /F %%i in ( 'powershell -Command "& {try{Invoke-RestMethod -Uri '%METADATA_HOST%/latest/api/token' -Method Put  -Headers @{'X-volc-ecs-metadata-token-ttl-seconds' = '600'}; } catch {exit 1;} }"') do ( set METADATA_TOKEN=%%i)
if %errorlevel% neq 0 (
   echo invalid metadata host
   exit 1
)
...
for /F %%i in ('powershell -Command "& {$wc = New-Object Net.WebClient;$wc.Headers.Add('X-volc-ecs-metadata-token', '%METADATA_TOKEN%'); $wc.DownloadString('%METADATA_HOST%/volcstack/latest/region_id'); }"') do ( set REGION_CODE=%%i)
echo REGION_CODE %REGION_CODE%

set REGION_ENDPOINT=%REGION_CODE%

if %REGION_CODE% == "cn-guilin-boe" (
    set REGION_ENDPOINT=cn-boe
)

set TOS_URL=http://assist-client-%REGION_CODE%.tos-%REGION_ENDPOINT%.ivolces.com
set TOS_BACKUP_URL=http://assist-client-%REGION_CODE%.tos-cn-boe.volces.com

call:getVersion

set DOWNLOAD_PATH=%TOS_URL%/%TAR_FILE%
set BACKUP_DOWNLOAD_PATH=%TOS_BACKUP_URL%/%TAR_FILE%

for /f "skip=3 tokens=4" %%i in ('sc query "assist-client"') do set "zf=%%i" &goto :next
:next
if /i "%zf%"=="" (
    echo this service is uninstall
) else (
    if /i "%zf%"=="RUNNING" (
       echo this service already installed
       exit 0
    )
)
:download
  if not exist C:\ProgramData md C:\ProgramData
  echo Download and Installing...
  powershell -Command "(New-Object Net.WebClient).DownloadFile('%DOWNLOAD_PATH%', 'C:\ProgramData\AssistClient.zip')" 1>nul 2>nul
  if %errorlevel% neq 0 (
     echo try backup download path
     powershell -Command "(New-Object Net.WebClient).DownloadFile('%BACKUP_DOWNLOAD_PATH%', 'C:\ProgramData\AssistClient.zip')" 1>nul 2>nul
  )
  if %errorlevel% neq 0 (
  ...
  powershell -Command "& {$b='C:\ProgramData\AssistClient.zip';$t='C:\ProgramData\AssistClient'; Add-Type -Assembly 'system.io.compression.filesystem'; [io.compression.zipfile]::ExtractToDirectory($b,$t);}"
  if %errorlevel% neq 0 (
     echo extract file failed
     del C:\ProgramData\AssistClient.zip
     exit 0
  )

  del C:\ProgramData\AssistClient.zip
  SC Create assist-client BinPath= "C:\ProgramData\AssistClient\assist-client.exe start" DisplayName= "assist-client" start= auto
  if %errorlevel%==0 (
     SC start assist-client
     sc failure assist-client reset=86400 actions=restart/60000/restart/120000/restart/120000
     schtasks /CREATE /tn check-assist-client /tr "C:\ProgramData\AssistClient\check.bat" /sc minute /mo 5 /ru System /f

     echo assist-client installed
     echo assist-client start
  ) else (
     echo create assist-client service is error
  )
goto:eof
GOTO END
...
:getVersion
    set DOWNLOAD_VERSION_PATH=%TOS_URL%/%VERSION_FILE%
    set BACKUP_VERSION_PATH=%TOS_BACKUP_URL%/%VERSION_FILE%

    if [%VERSION%] equ [] (
      for /F %%i in ('powershell -Command "& {$wc = New-Object Net.WebClient; $wc.DownloadString('%DOWNLOAD_VERSION_PATH%');}"') do ( set VERSION=%%i)
    )
    if [%VERSION%] equ [] (
      for /F %%i in ('powershell -Command "& {$wc = New-Object Net.WebClient; $wc.DownloadString('%BACKUP_VERSION_PATH%');}"') do ( set VERSION=%%i)
    )
    if [%VERSION%] equ [+] (
      for /F %%i in ('powershell -Command "& {$wc = New-Object Net.WebClient; $wc.DownloadString('%BACKUP_VERSION_PATH%');}"') do ( set VERSION=%%i)
    )
    ...
    set TAR_FILE=assist-client_windows_%ARCH%_%VERSION%.zip
goto:eof
